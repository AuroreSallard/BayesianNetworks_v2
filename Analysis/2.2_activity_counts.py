...
purposes = ["work", "education", "shopping", "leisure", "other"]
...
for ac in bn_data:
    ...
    for purpose in purposes:
        cpt = l.count(purpose)
        if cpt > 0 :
            identifier = purpose + " - " + str(cpt) 
            if cpt > 1:
                identifier += " times"
            else:
                identifier += " time"
            if identifier not in counts.keys():
                counts[identifier] = [0,0,0,0]
...
for i in range(len(mz_data)):
    ...
    for purpose in purposes:
        ...
        if cpt > 0 :
            ...
            if identifier not in counts.keys():
                counts[identifier] = [0,0,0,0]
            counts[identifier][2] += mz_weights[i]
...
for i in range(len(old_data)):
    ...
    for purpose in purposes:
        ...
        if cpt > 0 :
            ...
            if identifier not in counts.keys():
                counts[identifier] = [0,0,0,0]
            counts[identifier][3] += old_weights[i]
...
reversed_dic = {"Activities": activities, "BN Count": bn_count, "Eqasim Count": eq_count, "MZ Count": mz_count, "MZ old Count": old_count} 
...
top = 15
if not top is None:
    labels = df_act["Activities"].values.tolist()[:top]
...
#fig.tight_layout()
plt.plot()
plt.savefig("results/figures/BN_mix2_actchains_2.png")
